...
tab1, tab2, tab3, tab4 = st.tabs(["📊 สต็อกคงเหลือ", "📥 รับเข้าของแถม (IN)", "📤 ตัดจ่ายตามบิล (OUT)", "📈 รายงานเคลื่อนไหว (Report)"])
            ...
            with st.container(border=True):
                ...
                with c_info:
                    ...
                    if not df_freebie_logs.empty and str(row["sku"]) in df_freebie_logs["SKU"].values:
                        ...
                        col_i4.metric("รับเข้าล่าสุด", f"+{last_log['จำนวนที่รับเข้า']}", delta=f"วันที่ {last_log['วันที่']}")
                    else:
...
with tab2:
    ...
    is_new_sku = st.checkbox("➕ เพิ่มเป็น SKU ใหม่ / สินค้าใหม่", value=df_stock.empty)
    ...
    with st.form("form_in", clear_on_submit=False):
        ...
        submit_in = st.form_submit_button("💾 บันทึกรับเข้าของแถม", type="primary", use_container_width=True)
        ...
        if submit_in:
            if not target_sku or not target_name:
                ...
                st.success(f"✅ บันทึกรับเข้า/สร้างใหม่ '{target_name}' จำนวน +{qty_in} ชิ้นเรียบร้อย!")
                st.rerun()
    ...
    if not df_freebie_logs.empty and not df_stock.empty:
        for i, lrow in df_freebie_logs.iterrows():
            with st.expander(f"Log ID: {lrow['log_id']} | วันที่: {lrow['วันที่']} | SKU: {lrow['SKU']} | รับเข้า: +{lrow['จำนวนที่รับเข้า']}"):
                c_img_prev, c_ed1, c_del = st.columns([1.5, 2.5, 1.5])
                with c_img_prev:
                    if "file_obj" in lrow and pd.notna(lrow["file_obj"]) and str(lrow["file_obj"]) != "None" and os.path.exists(str(lrow["file_obj"])):
                        st.image(str(lrow["file_obj"]), width=90, caption="รูปปัจจุบัน")
                    else:
                        st.caption("ไม่มีรูป")
                
                with c_ed1:
                    edit_qty_val = st.number_input(f"แก้จำนวน (ID {lrow['log_id']})", min_value=1, value=int(lrow["จำนวนที่รับเข้า"]), key=f"in_qty_edit_{i}")
                    edit_file_val = st.file_uploader("แนบ/เปลี่ยนรูป (คอม/มือถือ)", type=["jpg", "jpeg", "png"], key=f"in_file_edit_{i}")
                    if st.button("💾 บันทึกแก้รับเข้า", key=f"save_in_edit_{i}"):
                        ...
                        if target_sku_str in df_stock["sku"].astype(str).values:
                            idx_s = df_stock[df_stock["sku"].astype(str) == target_sku_str].index[0]
                            df_stock.loc[idx_s, "qty"] = int(df_stock.loc[idx_s, "qty"]) + diff
                        df_freebie_logs.at[i, "จำนวนที่รับเข้า"] = edit_qty_val
                        if edit_file_val is not None:
                            os.makedirs("uploads", exist_ok=True)
                            new_path = os.path.join("uploads", f"{int(datetime.datetime.now().timestamp())}_{edit_file_val.name}")
                            with open(new_path, "wb") as f: f.write(edit_file_val.getbuffer())
...
with tab3:
    ...
    if df_stock.empty:
        ...
        with st.form("form_out", clear_on_submit=True):
            order_ref = st.text_input("เลขที่ออเดอร์ / Order Ref", placeholder="POS-20260901-001")
            display_list_out = (df_stock["sku"].astype(str) + " | " + df_stock["name"].astype(str)).tolist()
            selected_item_out = st.selectbox("เลือก SKU / ชื่อของแถม", display_list_out)
            ...
            location_out = st.text_input("ชื่อล็อกที่จัดของแถม", value=default_loc_out)
            qty_out = st.number_input("จำนวนที่แถมไป", min_value=1, value=1)
            
            submit_out = st.form_submit_button("ยืนยันตัดสต็อกของแถม", type="primary", use_container_width=True)
    ...
    if not df_out_logs.empty and not df_stock.empty:
        for i, orow in df_out_logs.iterrows():
            ...
            with st.expander(f"Log ID: {orow['log_id']} | วันที่: {orow['วันที่']} | ออเดอร์: {orow['เลขที่ออเดอร์']} | สินค้า: {item_name_str} | แจก: -{orow['จำนวนที่แถมไป']}"):
                c_ed_out, c_del_out = st.columns(2)
                with c_ed_out:
                    edit_q_out = st.number_input(f"แก้จำนวนแจก (ID {orow['log_id']})", min_value=1, value=int(orow["จำนวนที่แถมไป"]), key=f"out_qty_edit_{i}")
                    edit_order_out = st.text_input(f"แก้ออเดอร์ (ID {orow['log_id']})", value=str(orow["เลขที่ออเดอร์"]), key=f"out_ord_edit_{i}")
                    if st.button("💾 บันทึกแก้ตัดจ่าย", key=f"save_out_edit_{i}"):
                        ...
                        if diff > 0 and target_sku_str in df_stock["sku"].astype(str).values:
                            idx_s = df_stock[df_stock["sku"].astype(str) == target_sku_str].index[0]
...
with tab4:
    ...
    with r_col1:
        ...
        if not df_freebie_logs.empty:
            ...
            tot_in = filt_in["จำนวนที่รับเข้า"].sum() if not filt_in.empty else 0
            st.metric("รวมรับเข้าช่วงนี้", f"+{tot_in} ชิ้น")
            st.dataframe(filt_in[["log_id", "วันที่", "SKU", "ชื่อของแถม", "ชื่อล็อก", "จำนวนที่รับเข้า"]], use_container_width=True, hide_index=True)
            if not filt_in.empty:
                st.download_button("📥 โหลดรายงานรับเข้า (CSV)", filt_in.to_csv(index=False).encode('utf-8-sig'), "report_in.csv", mime="text/csv")
        else:
    ...
    with r_col2:
        ...
        if not df_out_logs.empty:
            ...
            tot_out = filt_out["จำนวนที่แถมไป"].sum() if not filt_out.empty else 0
            st.metric("รวมแถมออกช่วงนี้", f"-{tot_out} ชิ้น")
            st.dataframe(filt_out[["log_id", "วันที่", "เลขที่ออเดอร์", "SKU", "ชื่อของแถม", "ชื่อล็อก", "จำนวนที่แถมไป"]], use_container_width=True, hide_index=True)
            if not filt_out.empty:
                st.download_button("📤 โหลดรายงานแถมออก (CSV)", filt_out.to_csv(index=False).encode('utf-8-sig'), "report_out.csv", mime="text/csv")
        else:
